...
import subscribeme.models.oauth
...
OAUTH_APP_SETTINGS = {
    ...
    'twitter': {
        ...
        'consumer_key': 'JoXbUNEHDAQzXku2guGTKA',
        'consumer_secret': 'ImjWDQM5w6GMIJriyyr7hX6cZMg8QJ1fWOuvKZIWXI',
        ...
        'oauth_callback': 'http://localhost:8080/ui/index.html',
...
class OAuthClient(object):
    ...
    def get(self, api_method, http_method='GET', expected_status=(200,), **extra_params):
        ...
        if self.token is None:
            self.token = subscribeme.models.oauth.OAuthAccessToken.get_by_key_name(self.get_cookie())
    ...
    def get_request_token(self):
        ...
        token = subscribeme.models.oauth.OAuthRequestToken(
        ...
        self.handler.redirect(self.get_signed_url(
            self.service_info['user_auth_url'], token, 'POST', **oauth_callback
            ))
    ...
    def callback(self, return_to='/'):
        ...
        oauth_token = subscribeme.models.oauth.OAuthRequestToken.all().filter(
        ...
        self.token = subscribeme.models.oauth.OAuthAccessToken(
        ...
        if 'specifier_handler' in self.service_info:
            ...
            old = subscribeme.models.oauth.OAuthAccessToken.all().filter(
    ...
    def cleanup(self):
        query = subscribeme.models.oauth.OAuthRequestToken.all().filter(
...
class OAuthHandler(RequestHandler):
    ...
    def get(self, service, action=''):
        ...
        client = OAuthClient(service, self,
            oauth_callback=OAUTH_APP_SETTINGS[service]['oauth_callback'])
...
class FeedAPI(google.appengine.ext.webapp.RequestHandler):
    ...
    def get(self):
        token = subscribeme.models.oauth.OAuthAccessToken.get_by_key_name(
            self.request.get('id'))
        client = OAuthClient('twitter', self)
        url = client.get_signed_url(
            'https://api.twitter.com/1/statuses/home_timeline.atom',
            token, 'GET', count=100)
        fetch = google.appengine.api.urlfetch.fetch(url)
